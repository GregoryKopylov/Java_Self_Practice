...
public class Item {
    ...
    public void setName(String name) {
        if (name.isBlank() || name.isEmpty()) {
            System.err.println("Name can not be blanc or empty");
            System.exit(1);
        }
        if(!name.matches("^[a-zA-Z\\s]+$")) {
            System.err.println("Name can contain only letters and spaces");
        }
        this.name = name;
    }
    ...
    public void setUnitPrice(double unitPrice) {
        if (unitPrice < 0 ){
            System.err.println("Price can not be negative" + unitPrice);
            System.exit(1);
        }
